...
and ir_of_fun (f : func) (sym_tab : symbol_table) : llvm_ir * symbol_table * (ident list) =
  match f with
  |Proto(ret, id, args) ->
    if lookup sym_tab id != None then
      failwith("Error : Prototype of the already declared function " ^ id)
    else
      empty_ir, (FunctionSymbol {return_type=ret; identifier=id; arguments=args; state=Declared})::sym_tab, []
  ...
  |Func(ret, id, args, body) -> (match lookup sym_tab id with
    ...
    | Some(FunctionSymbol {return_type=ret2; arguments=args2; _}) -> 
      if not (verify_fun_compatibility ret ret2 args args2) then 
        ...
        ir_of_func ret id args body (remove_proto id sym_tab)
    | None ->
      ir_of_func ret id args body sym_tab
    )

and ir_of_func ret id args body sym_tab : llvm_ir * symbol_table * ident list =
  let st = (FunctionSymbol {return_type=ret; identifier=id; arguments=args; state=Defined})::sym_tab in
  ...
  ((empty_ir @^ struct_args) @: fun_head) @@ arg_ir @@ body_ir @: "ret i8* null\n}\n\n",
  st,
  called
...
and sym_tab_of_args (fun_id : ident) (args : params list) : llvm_ir * symbol_table =
  let rec aux fun_id struct_id args index : llvm_ir * symbol_table = 
    ...
    | (Var_params id)::tl -> let uniqid = newuniqid id in
                            ...
                            (empty_ir @: llvm_extract_var_arg uniqid fun_id struct_id index) @@ ir, VariableSymbol(Type_Int, id, uniqid)::st
    | (Tab_params id)::tl -> let uniqid = newuniqid id in
                             let ir, st = aux fun_id struct_id tl (index+1) in
                             (empty_ir @: llvm_extract_tab_arg uniqid fun_id struct_id index) @@ ir, VariableSymbol(Type_Tab_Ptr, id, uniqid)::st
  in let out = newtmp()
  in let ir, st = aux fun_id out args 0
  in (empty_ir @: llvm_args_struct out fun_id) @@ ir, st
...
and ir_of_instruction  (instr : instruction) (sym_tab : symbol_table) : llvm_ir * (ident list) = match instr with 
  ...
  |MapRed(tab, cut_count, tab_size, fun_id, args) -> let tids, ir, called2 = ir_of_map_red tab cut_count tab_size fun_id args sym_tab in
                                               let ir2 = ir_of_join_map_red tids in
                                               ir @@ ir2, called2
  |Join(tid, None) -> (match uniq_id_of_symbol_table sym_tab tid with
        | Some(Type_Tid, uniq_id) -> empty_ir @: llvm_join_void uniq_id, []
        | _ -> failwith("Error : JOIN first argument must be of type TID"))

  |Join(tid, Some(res_id)) -> (match (uniq_id_of_symbol_table sym_tab tid), (uniq_id_of_symbol_table sym_tab res_id) with
...
and ir_of_affect_var (var : ident) (value : llvm_value) (sym_tab : symbol_table): llvm_instr =
  ...
  | Some(Type_Tid, id) -> failwith ("Error : TID symbol " ^ var ^ " used as var symbol")
  | Some(Type_Int, uniq_id) -> llvm_affect_var value uniq_id

and ir_of_thread (tid : ident) (fun_id : ident) (args : expression list) (sym_tab : symbol_table) : llvm_ir * ident list = 
...
and ir_of_read (a : variable) (sym_tab : symbol_table) : llvm_ir * ident list = match a with
    | Var(ident) -> (match uniq_id_of_symbol_table sym_tab ident with
                    ...
                    | Some(Type_Tid, id) -> failwith ("Error : TID symbol " ^ ident ^ " used as var symbol")
...
and aux0_declaration (dec : declar ) (sym_tab : symbol_table): llvm_ir * symbol_table = match dec with 
  ...
  |DeclarationTid(a::q) -> let ir, st = aux_declaration_tid a sym_tab in
...
and ir_of_call (id : ident) (args : expression list) (sym_tab : symbol_table) : llvm_ir * ident list =
  ...
  | Some(FunctionSymbol {arguments=args2}) -> if verify_fun_call_compatibility T_Void T_Void args args2 sym_tab then
...
and compute_args (fun_id : ident) (args : expression list) (sym_tab : symbol_table) (called_func : ident list) : llvm_ir * ident * ident list =
  let rec aux (struct_id : ident) (args : expression list) (sym_tab : symbol_table) (called_func : ident list) (index : int) : llvm_ir * ident list =
    match args with
    | [] -> empty_ir, called_func
    | (Unit1(Unit0(VarExpression id)))::tl -> (match uniq_id_of_symbol_table sym_tab id with
                | Some(typ, un_id) when typ != Type_Int && typ != Type_Tid ->
                  ...
                  let arg_ptr_instr = llvm_arg_ptr_in_struct arg_ptr fun_id struct_id index in
                  let ptr_instr, out = instr_to_get_ptr id (LLVM_i32 0) sym_tab in
                  let store_arg = llvm_store_arg_in_struct (LLVM_var out) arg_ptr LLVM_type_i32_ptr in
                  let ir2, called = aux struct_id tl sym_tab called_func (index+1) in
                  (((empty_ir @: arg_ptr_instr) @: ptr_instr) @: store_arg) @@ ir2, called
                | _ -> 
                  let arg_ptr = newtmp() in
                  let arg_ptr_instr = llvm_arg_ptr_in_struct arg_ptr fun_id struct_id index in
                  let ir, arg_value, called = ir_of_expression (Unit1(Unit0(VarExpression id))) sym_tab in
                  let store_arg = llvm_store_arg_in_struct arg_value arg_ptr LLVM_type_i32 in
                  let ir2, called = aux struct_id tl sym_tab called_func (index+1) in
                  (((empty_ir @: arg_ptr_instr) @@ ir) @: store_arg) @@ ir2, called
      )
    | hd::tl -> let arg_ptr = newtmp() in
                let arg_ptr_instr = llvm_arg_ptr_in_struct arg_ptr fun_id struct_id index in
  ...
  in let tmp0 = newtmp()
  in let tmp1 = newtmp()
  in let args_storage, called = aux tmp0 args sym_tab called_func 0
  in (empty_ir @: llvm_args_init tmp0 fun_id) @@ args_storage @: llvm_convert_args_ptr tmp1 fun_id tmp0, tmp1, called


and ir_of_map_red tab cut_count tab_size fun_id args sym_tab : ident list * llvm_ir * ident list =
  let rec aux_map_red tab tab_size start map_size fun_id arg_values tids ir : ident list * llvm_ir =
    match tab_size - start with
    | n when n < 2 * map_size -> let tid = newtmp() in
                             let ir2 = llvm_map_red tid start (tab_size-start) tab tab_size fun_id arg_values in
                             tid::tids, ir@@ir2
    | _ -> let tid = newtmp() in
           let ir2 = llvm_map_red tid start map_size tab tab_size fun_id arg_values in
           aux_map_red tab tab_size (start+map_size) map_size fun_id arg_values (tid::tids) (ir@@ir2)
  in let rec aux_args ir args values called sym_tab : llvm_ir * ident list * (llvm_type * llvm_value) list =
    match args with
  ...
  (match uniq_id_of_symbol_table sym_tab id with
      | Some(typ, uniq_id) when typ != Type_Int && typ != Type_Tid -> 
        let ptr_instr, out = instr_to_get_ptr id (LLVM_i32 0) sym_tab in
        aux_args (ir@:ptr_instr) tl ((LLVM_type_i32_ptr, LLVM_var out)::values) called sym_tab
      | Some(_) -> let ir2, value, called2 = ir_of_expression (Unit1(Unit0(VarExpression(id)))) sym_tab in
        aux_args (ir @@ ir2) tl ((LLVM_type_i32, value)::values) (called@called2) sym_tab
      | None -> failwith("Error : unknown var " ^ id) 
    )
    | hd::tl -> let ir2, value, called2 = ir_of_expression hd sym_tab in
      aux_args (ir @@ ir2) tl ((LLVM_type_i32, value)::values) (called@called2) sym_tab
  in match uniq_id_of_symbol_table sym_tab tab with
  | Some(Type_Int, _) -> failwith("Error : first argument of MAP must be of type INT ARRAY, but " ^ tab ^ " is of type INT")
  | Some(Type_Tid, _) -> failwith("Error : first argument of MAP must be of type INT ARRAY, but " ^ tab ^ " is of type TID")
  | None -> failwith("Error : undefined var " ^ tab)
  | Some(_, uniqid) -> match lookup sym_tab fun_id with
      | Some(FunctionSymbol({arguments = (Tab_params tab)::(Var_params size)::tl})) -> 
        ...
        let tids, ir2 = aux_map_red uniqid tab_size 0 (tab_size/cut_count) fun_id arg_values [] empty_ir in
        tids, ir@@ir2, called2
      | Some(FunctionSymbol(_)) -> failwith("TODO")
      | Some(VariableSymbol _) -> failwith("Error : TODO")
      | None -> failwith("Error : TODO")
...
and aux_declaration_tid (var : decl_variable ) (sym_tab : symbol_table) : llvm_instr * symbol_table = match var with 
  |DVar(id)-> if lookup sym_tab id != None then
                failwith("Error : multiple déclaration of " ^ id)
              else
                let un_id = newuniqid id in
...
and ir_of_call_expr (id : ident) (args : expression list) (sym_tab : symbol_table) (called_func : ident list): llvm_ir * llvm_value * ident list = 
  ...
  | Some(FunctionSymbol {arguments=args2}) -> if verify_fun_call_compatibility T_Int T_Int args args2 sym_tab then
                                                ...
                                                ir @: (llvm_call_expr fun_dest id arg_struct), LLVM_var fun_dest, id::called
                                              else
                                                failwith("Error : Incompatible arguments type and/or number in declaration and call of function " ^ id)  
...
and instr_to_get_ptr tab index sym_tab : llvm_instr * llvm_var =
  ...
  | Some(Type_Int, uniq_id) -> failwith("Error : var symbol " ^ tab ^ " used as tab symbol")
  | Some(Type_Tid, uniq_id) -> failwith("Error : TID symbol " ^ tab ^ " used as tab symbol")
