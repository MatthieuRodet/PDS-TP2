...
and ir_of_prog (prog :  program) (sym_tab : symbol_table): llvm_ir * symbol_table = match prog with 
  |Prog([]) ->  empty_ir, []
  |Prog(a::q) -> let ir, decl = ir_of_fun a sym_tab in
                 let ir2, decl2 = ir_of_prog (Prog q) ((FunctionSymbol decl)::sym_tab) in
                 ir @@ ir2, (FunctionSymbol decl)::decl2
...
and ir_of_block (b : block) (sym_tab : symbol_table): llvm_ir = match b with 
  |Unit(declar, instr) -> let ir, st = ir_of_declaration declar [] in ir @@ ir_of_instructions instr (st@sym_tab)
...
and ir_of_call (id : ident) (args : expression list) (sym_tab : symbol_table) : llvm_ir =
  ...
  | Some(FunctionSymbol {arguments=args2}) -> if verify_fun_call_compatibility T_Int T_Int args args2 sym_tab then
                                                let ir, args_values = compute_args args sym_tab in
